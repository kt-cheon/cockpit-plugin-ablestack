...
def map_links_by_dm(dm_devices):
    ...
    for entry in os.listdir(by_id_path):
        ...
        if target_basename not in dm_devices:
            continue

        # dm-* 엔트리 초기화(type은 내부에 넣지 않습니다)
    ...
    # type을 dm-5와 같은 최상위 레벨에 추가합니다
    return {**sorted_dm_map}

# /dev/disk/by-id 내부 링크 분류 및 매핑 (싱글 디스크용)

def map_links_by_single(single_devices: Iterable[str]) -> Dict[str, Any]:
    """
    /dev/disk/by-id 아래의 심볼릭 링크들을 순회하면서
    대상이 되는 단일 디스크 이름(single_devices)에 매핑합니다.
    최상위(외부)에 {"type": "single"}을 두고, 장치들은 같은 레벨로 반환합니다.
    """
    by_id_path = "/dev/disk/by-id"
    single_map: Dict[str, Any] = {}
    ...
    for entry in os.listdir(by_id_path):
        ...
        # 장치별 초기 구조 생성(type은 내부에 넣지 않습니다)
    ...
    # 장치 이름 기준 정렬
    sorted_single_map = dict(sorted(single_map.items(), key=lambda x: x[0]))

    # 최상위(외부)에 type을 추가합니다
...
def result():
    try:
        ...
        if mpath_status == "active":
            ...
            return createReturn(code=200, val=mapped, type_str="multipath")
        else:
            single_devices = get_single_devices()
            mapped = map_links_by_single(single_devices)

            return createReturn(code=200, val=mapped, type_str="single")
